...
export interface TwinBindingInput {
  twinObjectId: string; deviceType: TwinBinding["deviceType"]; deviceId: string;
...
export class MemoryDigitalTwinState implements DigitalTwinState {
  ...
  async bindDevice(input: TwinBindingInput) {
    for (const binding of this.bindings.values()) if (binding.deviceType === input.deviceType && binding.deviceId === input.deviceId && binding.twinObjectId !== input.twinObjectId) throw new Error("device_already_bound");
    const existing = this.bindingFor(input.twinObjectId);
    const now = new Date().toISOString();
    const value: TwinBinding = { id: existing?.id ?? randomUUID(), twinObjectId: input.twinObjectId, deviceType: input.deviceType, deviceId: input.deviceId, statusSource: input.statusSource ?? null, alertSource: input.alertSource ?? null, autoUpdate: input.autoUpdate ?? true, metadata: input.metadata ?? {} };
...
class PostgresDigitalTwinState implements DigitalTwinState {
  ...
  async bindDevice(input: TwinBindingInput) { const row = (await this.pool.query(`INSERT INTO digital_twin_device_bindings (twin_object_id,device_type,device_id,device_table,status_source,alert_source,auto_update,metadata) VALUES ($1,$2,$3,'control_plane',$4,$5,$6,$7) ON CONFLICT (twin_object_id,device_type,device_id) DO UPDATE SET status_source=EXCLUDED.status_source,alert_source=EXCLUDED.alert_source,auto_update=EXCLUDED.auto_update,metadata=EXCLUDED.metadata,updated_at=now() RETURNING *`, [input.twinObjectId, input.deviceType, input.deviceId, input.statusSource ?? null, input.alertSource ?? null, input.autoUpdate ?? true, JSON.stringify(input.metadata ?? {})])).rows[0]; return mapBinding(row); }
...
function objectSelect(where: string) { return `SELECT o.*,b.id binding_id,b.device_type,b.device_id,b.status_source,b.alert_source,b.auto_update,b.metadata binding_metadata FROM digital_twin_objects o LEFT JOIN digital_twin_device_bindings b ON b.twin_object_id=o.id WHERE ${where}`; }
...
function mapJoinedObject(r:any){return mapObject(r,r.binding_id?{id:r.binding_id,twinObjectId:r.id,deviceType:r.device_type,deviceId:r.device_id,statusSource:r.status_source??null,alertSource:r.alert_source??null,autoUpdate:Boolean(r.auto_update),metadata:r.binding_metadata??{}}:null);}
function mapBinding(r:any):TwinBinding{return{id:r.id,twinObjectId:r.twin_object_id,deviceType:r.device_type,deviceId:r.device_id,statusSource:r.status_source??null,alertSource:r.alert_source??null,autoUpdate:Boolean(r.auto_update),metadata:r.metadata??{}};}
