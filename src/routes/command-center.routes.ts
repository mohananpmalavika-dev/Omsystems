...
import { correlateBranchDiagnoses } from "../services/command-center/correlation.js";
...
export async function registerCommandCenterRoutes(app: FastifyInstance, store: ControlPlaneStore) {
  ...
  app.get("/v1/command-center/rca/multi-branch-analysis", async (request, reply) => commandReply(reply, async () => {
    const user = authenticated(request);
    const query = z.object({
      branchIds: z.string().min(1).transform((value, context) => {
        const ids = [...new Set(value.split(",").map((id) => id.trim()).filter(Boolean))];
        if (ids.length < 2 || ids.length > 100) {
          context.addIssue({ code: z.ZodIssueCode.custom, message: "Provide between 2 and 100 unique branch IDs" });
          return z.NEVER;
        }
        return ids;
      }),
    ...
    const diagnoses = await Promise.all(query.branchIds.map((branchId) =>
      service.diagnosis(user, branchId, {
        ...(query.from ? { from: query.from } : {}),
        ...(query.to ? { to: query.to } : {}),
      }),
    ));
    const analysis = correlateBranchDiagnoses(diagnoses);
    await audit(store, request, "command_center.multi_branch_analysis.read", query.branchIds[0]!, "success", {
      branchIds: query.branchIds,
      rootCauseClusterCount: analysis.rootCauseClusters.length,
      crossBranchSignalCount: analysis.crossBranchSignals.length,
    });
    return {
      branchIds: query.branchIds,
      analysisWindow: { from: query.from ?? null, to: query.to ?? null },
      ...analysis,
      generatedAt: new Date().toISOString(),
    };
  }));
