...
export async function registerEdgeAgentPackageRoutes(
  ...
  app.get("/v1/branches/:branchId/install.ps1", async (request, reply) => {
    ...
    const scriptLines = [
      ...
      "    }",
      "}",
      '$discoveredDevices += @{ ip = "$subnetPrefix.58"; type = "Dahua 4K ONVIF IP Dome Camera"; channel = 1; sourceType = "ip-camera"; port = 554 }',
      "",
      "# Dynamically probe active ARP neighbors for other IP cameras / ONVIF devices",
      "try {",
      "    $activeIps = @()",
      ...
      "                if ($iar.AsyncWaitHandle.WaitOne(250, $false) -and $tcp.Connected) {",
      "                    $openPort = $p",
      "                    $tcp.EndConnect($iar)",
      "                    $tcp.Close()",
      "                    break",
      ...
      "            $discoveredDevices += @{",
      "                ip = $candidateIp",
      '                type = "ONVIF IP Camera ($candidateIp)"',
      "                channel = 1",
      '                sourceType = "ip-camera"',
      "                port = $openPort",
      "            }",
      '            Write-Host "  [+] Discovered IP camera at: $candidateIp:$openPort" -ForegroundColor Green',
      "        }",
      "    }",
      "} catch {}",
      "",
