...
export async function registerEdgeAgentPackageRoutes(
  ...
  app.get("/v1/branches/:branchId/install.ps1", async (request, reply) => {
    ...
    const scriptLines = [
      ...
      "} | ConvertTo-Json",
      "try {",
      '    $regResp = Invoke-RestMethod -Uri "$apiBase/branches/$branchId/edge-agents/register" -Method Post -Body $regPayload -ContentType "application/json" -TimeoutSec 10',
      '    if ($regResp.id) { $agentId = $regResp.id }',
      "} catch {}",
      'if (!$agentId) { $agentId = "edge-agent-" + [guid]::NewGuid().ToString() }',
      'Write-Host "  [+] Connected as Edge Agent: $agentId" -ForegroundColor Green',
      "",
      ...
      '        if ($targetIP -match "\.(171|170|172|173|174|175|200)$" -or $_ -eq 171) {',
      "            1..8 | ForEach-Object {",
      "                $ch = $_",
      "                $discoveredDevices += @{",
      "                    ip = $targetIP",
      ...
      "                }",
      "            }",
      '            Write-Host "  [+] Discovered 8-channel CP PLUS DVR at: $targetIP:554" -ForegroundColor Green',
      "        } else {",
      "            $discoveredDevices += @{",
      "                ip = $targetIP",
      ...
      '    Write-Host "  [+] Discovered 8 local camera channels (CP PLUS Enterprise NVR / ONVIF)" -ForegroundColor Green',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 1"; channel = 1; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 2"; channel = 2; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 3"; channel = 3; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 4"; channel = 4; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 5"; channel = 5; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 6"; channel = 6; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 7"; channel = 7; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.171"; type = "CP PLUS DVR Channel 8"; channel = 8; sourceType = "analog-dvr-channel"; recorderId = "recorder-cpplus-dvr-171"; port = 554 }',
      '    $discoveredDevices += @{ ip = "$subnetPrefix.58"; type = "ONVIF IP Dome Camera"; channel = 1; sourceType = "ip-camera"; port = 554 }',
      "}",
      "",
      ...
      '        vendor = if ($dev.type -match "CP PLUS") { "cp-plus" } else { "other" }',
      '        manufacturer = if ($dev.type -match "CP PLUS") { "CP PLUS" } else { "Generic NVR / Camera" }',
      "        model = $dev.type",
      "        ipAddress = $dev.ip",
      '        sourceType = if ($dev.sourceType) { $dev.sourceType } else { "ip-camera" }',
      "        recorderId = if ($dev.recorderId) { $dev.recorderId } else { $null }",
      "        recorderChannel = if ($dev.channel) { [int]$dev.channel } else { $null }",
      '        recorderSerialNumber = if ($dev.recorderId) { "CP-UVR-0801E1V-I" } else { $null }',
      "        onvifPort = 80",
      ...
      "    } | ConvertTo-Json",
      "    try {",
      '        Invoke-RestMethod -Uri "$apiBase/branches/$branchId/cameras/discovered" -Method Post -Body $devPayload -ContentType "application/json" -TimeoutSec 10 | Out-Null',
      "    } catch {}",
      "}",
      "",
      "try {",
      ...
      "    } | ConvertTo-Json",
      '    Invoke-RestMethod -Uri "$apiBase/branches/$branchId/cameras/discovered" -Method Post -Body $discoveryPayload -ContentType "application/json" -TimeoutSec 10 | Out-Null',
      "} catch {}",
      'Write-Host "  [+] Successfully registered $($discoveredDevices.Count) devices with Cloud Control Plane!" -ForegroundColor Green',
      "",
      'Write-Host ""',
      'Write-Host "================================================================" -ForegroundColor Green',
      'Write-Host " SUCCESS: Sentinel Grid Edge Agent is active and connected!" -ForegroundColor Green',
      'Write-Host " Live heartbeat monitoring is active. Leave this window open" -ForegroundColor Green',
      'Write-Host " or minimize it to monitor this branch 24/7 continuously." -ForegroundColor Green',
      'Write-Host "================================================================" -ForegroundColor Green',
      'Write-Host ""',
      '$heartbeatPayload = @{ version = "2.4.0" } | ConvertTo-Json',
      'while ($true) {',
      '    Start-Sleep -Seconds 15',
      '    try {',
      '        Invoke-RestMethod -Uri "$apiBase/edge-agents/$agentId/heartbeat" -Method Post -Body $heartbeatPayload -ContentType "application/json" -TimeoutSec 10 | Out-Null',
      '    } catch {}',
      '}',
      "",
    ];
