...
export async function registerEdgeAgentPackageRoutes(
  ...
  app.get("/v1/branches/:branchId/install.ps1", async (request, reply) => {
    ...
    const scriptLines = [
      ...
      '    try {',
      '        Invoke-RestMethod -Uri "$controlPlaneUrl/api/control/v1/edge-agents/$agentId/heartbeat" -Method Post -Body $heartbeatPayload -ContentType "application/json" -TimeoutSec 10 | Out-Null',
      '    } catch {}',
      '}',
      "",
    ];
