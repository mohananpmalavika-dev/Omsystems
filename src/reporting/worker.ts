...
function csv(report:DailyOperationalReport,template:OperationalReportTemplate){const rows=reportSections(report,template).flatMap((section)=>section.rows.map((row)=>({recordType:section.recordType,...row})));const headers=[...new Set(rows.flatMap((row)=>Object.keys(row)))];return [headers.join(","),...rows.map((row)=>{const rowObject = row as Record<string, unknown>; return headers.map((key)=>escapeCsv(rowObject[key])).join(",");})].join("\r\n");}
function escapeCsv(value:unknown){const text=value==null?"":String(value);return /[",\r\n]/.test(text)?`"${text.replaceAll('"','""')}"`:text;}
async function xlsx(report:DailyOperationalReport,template:OperationalReportTemplate){const workbook=new ExcelJS.Workbook();workbook.creator="KryptoVision";const summary=workbook.addWorksheet("Summary");summary.addRow(["Template",humanize(template)]);summary.addRow(["Metric","Value"]);for(const [key,value] of Object.entries(report.summary))summary.addRow([key,value]);for(const {name,rows} of reportSections(report,template)){const sheet=workbook.addWorksheet(name);const headers=[...new Set(rows.flatMap((row)=>Object.keys(row)))];sheet.addRow(headers);for(const row of rows){const rowObject=row as Record<string, unknown>; sheet.addRow(headers.map((key)=>rowObject[key]));}sheet.views=[{state:"frozen",ySplit:1}];sheet.autoFilter=headers.length?{from:{row:1,column:1},to:{row:Math.max(1,rows.length+1),column:headers.length}}:undefined;sheet.getRow(1).font={bold:true,color:{argb:"FFFFFFFF"}};sheet.getRow(1).fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF1E3A8A"}};}return Buffer.from(await workbook.xlsx.writeBuffer());}
