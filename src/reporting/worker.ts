...
export async function buildDailyOperationalReport(store:ControlPlaneStore,user:NonNullable<Awaited<ReturnType<ControlPlaneStore["getUser"]>>>,filters:OperationalReportRun["filters"]):Promise<DailyOperationalReport>{
  ...
  for(const branch of branches){const cameras=await store.listCamerasByBranch(user,branch.id,"live:view");const policy=await store.getOperationalHealthPolicy(user.tenantId,branch.id)??await store.getOperationalHealthPolicy(user.tenantId)??defaultOperationalHealthPolicy;const retentions=await Promise.all(cameras.map(async(camera)=>verifyContinuousRetention(camera.id,await store.listRecordingSegments(camera.id),policy,Date.parse(to))));projections.push(projectBranchHealth({branch,cameras,telemetry:telemetry.filter((item)=>item.branchId===branch.id),retentions,policy,now:Date.parse(to),region:regionByBranch.get(branch.id)}));}
...
export function reportSections(report:DailyOperationalReport,template:OperationalReportTemplate){
  const all=[{name:"Branches",recordType:"branch",rows:report.branches},{name:"Cameras",recordType:"camera",rows:report.cameras},{name:"Alerts",recordType:"alert",rows:report.alerts},{name:"Exceptions",recordType:"exception",rows:report.exceptions}];
