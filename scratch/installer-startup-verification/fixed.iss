...
CreateAppDir=no
Uninstallable=no
OutputDir=C:/Omsystems/scratch/installer-startup-verification
OutputBaseFilename=startup-fixed
[Code]
...
procedure InitializeAgentWizard;
begin
...
procedure DisabledInstallStep(CurStep: TSetupStep);
begin
...
begin
  Log('STARTUP_PROBE_BEGIN');
  InitializeAgentWizard;
  Log('STARTUP_PROBE_PASSED');
  Abort;
end;
procedure CurStepChanged(CurStep: TSetupStep);
begin
  RaiseException('Startup probe must never install files');
end;