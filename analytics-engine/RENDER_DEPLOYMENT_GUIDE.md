...
**Service URL:** https://sentinel-grid-analytics-engine-u2sf.onrender.com
...
```bash
curl https://sentinel-grid-analytics-engine-u2sf.onrender.com/health | jq '.aiState'
...
# Full health check
curl https://sentinel-grid-analytics-engine-u2sf.onrender.com/health | jq

# Per-camera status (requires authentication)
curl -H "x-analytics-source-key: YOUR_KEY" \
  https://sentinel-grid-analytics-engine-u2sf.onrender.com/v1/analytics/cameras/cam123/status
```
