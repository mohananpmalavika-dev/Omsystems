...
const schema = z.object({
    ...
    CLOUDFLARED_TUNNEL_TOKEN: z.preprocess((value) => value === "" ? undefined : value, z.string().min(20).optional()),
    ...
    PUBLIC_MEDIA_GATEWAY_URL: z.preprocess((value) => value === "" ? undefined : value, z.string().url().optional()),
    ...
    EDGE_MEDIA_SHARED_KEY: z.preprocess((value) => value === "" ? undefined : value, z.string().min(32).optional()),
    EDGE_BRIDGE_SHARED_KEY: z.preprocess((value) => value === "" ? undefined : value, z.string().min(32).optional()),
    ...
    INTERNET_LINKS_JSON: z.string().default("[]").transform((value, context) => {
        try {
            ...
            context.addIssue({ code: z.ZodIssueCode.custom, message: "INTERNET_LINKS_JSON must be valid JSON" });
            return z.NEVER;
        }
    ...
    RECORDERS_JSON: z.string().default("[]").transform((value, context) => {
        try {
            ...
            context.addIssue({ code: z.ZodIssueCode.custom, message: "RECORDERS_JSON must be valid JSON" });
            return z.NEVER;
        }
    }).pipe(z.array(z.object({
            ...
            channels: z.array(z.object({
                ...
                channels.forEach((item, index) => {
                    if (cameraIds.has(item.cameraId))
                        context.addIssue({ code: z.ZodIssueCode.custom, path: [index, "cameraId"], message: "Each cameraId may be mapped once per recorder" });
                    if (channelIds.has(item.channel))
                        context.addIssue({ code: z.ZodIssueCode.custom, path: [index, "channel"], message: "Each recorder channel may be mapped once" });
...
export function loadEdgeConfig(environment = process.env) {
