...
export function inspectBundledWindowsRuntime() {
    return REQUIRED_BUNDLE_ASSETS.map(([name, path]) => {
        if (!existsSync(path))
            throw new Error(`The all-in-one installer is missing ${name}`);
        const sizeBytes = statSync(path).size;
        if (sizeBytes <= 0)
            throw new Error(`The bundled ${name} is empty`);
        return { name, sizeBytes };
    });
}
export function launchWindowsSelfInstaller(environmentFile) {
    if (process.platform !== "win32")
        throw new Error("The embedded installer can only run on Windows");
    ...
    try {
        ...
        if (result.error)
            throw result.error;
        if (result.status !== 0)
            throw new Error(`Installation was cancelled or failed (exit ${result.status ?? "unknown"})`);
    }
    finally {
        rmSync(stage, { recursive: true, force: true });
    }
...
function copyAsset(source, destination) {
    if (!existsSync(source))
        throw new Error(`The all-in-one installer is missing ${source.split(/[\\/]/).at(-1)}`);
