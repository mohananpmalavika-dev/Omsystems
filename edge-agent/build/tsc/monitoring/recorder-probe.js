...
export async function probeRecorder(config, timeoutMs, options = {}) {
    ...
    try {
        const family = recorderApiFamily(config);
        if (family === "hikvision-isapi")
            return await probeHikvision(config, base, credentials, timeoutMs, started, Boolean(options.includeArchive));
        if (family === "dahua-cgi")
            return await probeDahuaFamily(config, base, credentials, timeoutMs, started, Boolean(options.includeArchive));
        if (family === "onvif")
            return await probeOnvif(config, base, credentials, timeoutMs, started, Boolean(options.includeArchive));
 ...
 */
export function recorderApiFamily(config) {
    if (config.apiFamily)
        return config.apiFamily;
...
async function probeHikvision(config, base, credentials, timeout, started, includeArchive) {
    ...
    if (system.status === 401 || system.status === 403)
        return result(config, true, "degraded", started, {}, [], ["recorder_credentials_rejected"]);
    if (!system.ok)
        throw new Error(`hikvision_http_${system.status}`);
...
async function getHikvisionRecordingStatus(base, credentials, timeout, channels) {
    try {
        ...
        if (!response.ok)
            return recordingUnavailable("hikvision_recording_search_unavailable", channels);
...
async function probeDahuaFamily(config, base, credentials, timeout, started, includeArchive) {
    ...
    if (system.status === 401 || system.status === 403)
        return result(config, true, "degraded", started, {}, [], ["recorder_credentials_rejected"]);
    if (!system.ok)
        throw new Error(`${config.vendor}_http_${system.status}`);
...
async function getDahuaRecordingStatus(base, credentials, timeout, channels) {
    ...
    try {
        ...
        if (!factory.ok)
            return recordingUnavailable("dahua_archive_search_unavailable", channels);
        object = key(await factory.text(), "object");
        if (!object)
            return recordingUnavailable("dahua_archive_search_handle_missing", channels);
        const now = new Date();
        ...
        for (let page = 0; page < 40; page++) {
            ...
            if (!next.ok)
                return recordingUnavailable("dahua_archive_results_unavailable", channels);
            const text = await next.text();
            const found = key(text, "found");
            if (!dahuaFoundResults(found))
                return recordingProbeFromMatches(matches, channels, "dahua_no_recent_recording_evidence");
            const parsed = parseDahuaRecordingMatches(text);
            if (!parsed.length)
                return recordingUnavailable("dahua_archive_results_unparseable", channels);
            matches.push(...parsed);
        }
...
async function searchHikvisionArchive(base, credentials, timeout, trackId, from, to, maxResults) {
    ...
    for (let position = 0; segments.length < maxResults; position += pageSize) {
        ...
        if (!response.ok)
            throw new Error(`hikvision_archive_${response.status}`);
...
async function searchDahuaArchive(base, credentials, timeout, channel, from, to, maxResults) {
    ...
    try {
        const factory = await authenticatedFetch(`${base}/cgi-bin/mediaFileFind.cgi?action=factory.create`, { method: "GET" }, credentials, timeout);
        if (!factory.ok)
            throw new Error(`dahua_archive_factory_${factory.status}`);
        object = key(await factory.text(), "object");
        if (!object)
            throw new Error("dahua_archive_handle_missing");
        const query = new URLSearchParams({
            action: "findFile", object,
        ...
        const find = await authenticatedFetch(`${base}/cgi-bin/mediaFileFind.cgi?${query}`, { method: "GET" }, credentials, timeout);
        if (!find.ok)
            throw new Error(`dahua_archive_find_${find.status}`);
        const pageSize = Math.min(1_000, maxResults);
        while (segments.length < maxResults) {
            ...
            if (!next.ok)
                throw new Error(`dahua_archive_next_${next.status}`);
            const text = await next.text();
            const found = key(text, "found");
            ...
            if (!dahuaFoundResults(found))
                return { segments, coverageComplete: true, reasonCodes: [] };
            if (page.length === 0)
                return { segments, coverageComplete: false, reasonCodes: ["dahua_archive_retention_unparseable"] };
            segments.push(...page);
        }
...
function archiveEvidenceFromSearch(mapping, result, retention, searchFrom, searchTo) {
...
function continuousArchiveWindow(segments, gapSeconds) {
    ...
    for (const segment of ordered.slice(1)) {
        if (cursor - segment.endedAt > gapSeconds * 1_000)
            break;
...
async function mapArchiveChannels(channels, mapper) {
    ...
    const workers = Array.from({ length: Math.min(4, channels.length) }, async () => {
        while (pending.length) {
            ...
            if (channel)
                results.push(await mapper(channel));
        }
    });
...
function parseDahuaArchiveSegments(text) {
    ...
    for (const match of text.matchAll(/(?:items|item)\[(\d+)\]\.(StartTime|EndTime|Channel)=([^\r\n]+)/gi)) {
        ...
        if (match[2].toLowerCase() === "starttime")
            item.startedAt = match[3].trim();
        else if (match[2].toLowerCase() === "endtime")
            item.endedAt = match[3].trim();
        else
            item.sourceChannel = match[3].trim();
...
async function probeOnvif(config, base, credentials, timeout, started, includeArchive) {
    ...
    if (response.status === 401 || response.status === 403)
        return result(config, true, "degraded", started, {}, [], ["recorder_credentials_rejected"]);
    if (!response.ok)
        throw new Error(`onvif_http_${response.status}`);
    const xml = await response.text();
...
async function getOnvifRecordingStatus(base, deviceServicePath, credentials, timeout) {
    try {
        ...
        if (!summary)
            return recordingUnavailable("onvif_recording_search_unavailable");
        const lastRecordedAt = newestOnvifRecordingData(summary);
        if (hasRecentOnvifRecordingData(summary))
            return { status: "recording", recordingChannels: null, lastRecordedAt, channels: [], reasonCodes: [], source: "recording-summary" };
...
function hikvisionArchiveSearchBody(trackIds, start = new Date(Date.now() - RECORDING_EVIDENCE_WINDOW_MS), end = new Date(), maxResults = 128, searchResultPosition = 0) {
...
function parseDahuaRecordingMatches(text) {
    ...
    for (const match of text.matchAll(/(?:items|item)\[(\d+)\]\.(Channel|EndTime)=([^\r\n]+)/gi)) {
        const item = grouped.get(match[1]) ?? {};
        if (match[2].toLowerCase() === "channel")
            item.sourceChannel = Number(match[3].trim());
        else {
...
function recordingProbeFromMatches(matches, inventory, stoppedReason) {
    ...
    const status = aggregateOnlyMatch
        ? "recording"
        : recordingChannels === 0
            ? "stopped"
...
function dahuaTime(value) { return value.toISOString().replace("T", " ").replace(/\.\d{3}Z$/, ""); }
...
function hasRecentOnvifRecordingData(xml) {
    ...
    if (Number.isFinite(numberRecordings) && numberRecordings === 0)
        return false;
...
function hikvisionChannelInventory(channelXml, statusXml) {
    ...
    for (const match of statusXml.matchAll(/<(?:[^:>]+:)?InputProxyChannelStatus\b[^>]*>([\s\S]*?)<\/(?:[^:>]+:)?InputProxyChannelStatus>/gi)) {
        ...
        if (Number.isInteger(id) && (online === "true" || online === "false"))
            statusByChannel.set(id, online === "true");
    }
...
function hikvisionTrackId(sourceChannel) { return String(sourceChannel >= 100 ? sourceChannel : sourceChannel * 100 + 1); }
function hikvisionSourceChannel(trackId) { return Number.isInteger(trackId) && trackId > 0 ? (trackId >= 100 ? Math.floor(trackId / 100) : trackId) : null; }
function dahuaVideoLossChannels(text) {
    ...
    for (const match of text.matchAll(/(?:result|index(?:es)?)\s*=\s*([^\r\n]*)/gi)) {
        for (const value of match[1].match(/\d+/g) ?? [])
            channels.add(Number(value));
    }
    for (const match of text.matchAll(/(?:channels?|index(?:es)?)\[(\d+)\]\s*=\s*(?:true|1)/gi))
        channels.add(Number(match[1]));
    return channels;
}
...
function tag(xml, name) { return xml.match(new RegExp(`<(?:[^:>]+:)?${name}>([^<]+)<\\/(?:[^:>]+:)?${name}>`, "i"))?.[1]; }
function firstTag(xml, names) { return names.map((name) => tag(xml, name)).find((value) => Boolean(value)); }
function key(text, name) { return text.match(new RegExp(`(?:^|\\n)${name}=([^\\r\\n]+)`, "i"))?.[1]?.trim(); }
