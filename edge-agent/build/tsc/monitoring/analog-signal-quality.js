 ...
 */
export function assessAnalogRgbFrame(previous, frame, width = 64, height = 36) {
    if (frame.length !== width * height * 3)
        throw new Error("invalid_rgb_frame_size");
    ...
    const rowInterferenceScore = rowMeans.slice(1).reduce((sum, value, index) => sum + Math.abs(value - rowMeans[index]), 0) / Math.max(1, height - 1);
