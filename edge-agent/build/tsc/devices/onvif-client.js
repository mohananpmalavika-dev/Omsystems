...
export class OnvifClient {
    ...
    constructor(deviceServiceUrl, credentials, timeoutMs = 8000) {
    ...
    async inspect() {
        ...
        const info = await this.call(this.deviceServiceUrl, "http://www.onvif.org/ver10/device/wsdl/GetDeviceInformation", `<tds:GetDeviceInformation/>`, `xmlns:tds="http://www.onvif.org/ver10/device/wsdl"`);
        const capabilities = await this.call(this.deviceServiceUrl, "http://www.onvif.org/ver10/device/wsdl/GetCapabilities", `<tds:GetCapabilities><tds:Category>All</tds:Category></tds:GetCapabilities>`, `xmlns:tds="http://www.onvif.org/ver10/device/wsdl"`);
        ...
        if (!mediaServiceUrl)
            throw new Error("ONVIF device did not provide a media service URL");
        const profileDocument = await this.call(mediaServiceUrl, "http://www.onvif.org/ver10/media/wsdl/GetProfiles", `<trt:GetProfiles/>`, `xmlns:trt="http://www.onvif.org/ver10/media/wsdl"`);
    ...
    async getStreamUri(mediaServiceUrl, profileToken) {
        const document = await this.call(mediaServiceUrl, "http://www.onvif.org/ver10/media/wsdl/GetStreamUri", `<trt:GetStreamUri>
        <trt:StreamSetup>
       ...
       xmlns:tt="http://www.onvif.org/ver10/schema"`);
        ...
        if (!uri)
            throw new Error("ONVIF profile did not return an RTSP URI");
        return uri;
    }
    async call(url, action, body, namespaces) {
        ...
        try {
            ...
            if (fault)
                throw new Error(`ONVIF SOAP fault: ${JSON.stringify(fault).slice(0, 300)}`);
            return parsed;
        }
        finally {
...
export function attachCredentials(uri, credentials) {
...
export function redactStreamUri(uri) {
    ...
    parsed.username = "";
    parsed.password = "";
    parsed.pathname = parsed.pathname.replace(/([/_](?:user|username|password)=)[^_/?&]*/gi, "$1[redacted]");
...
function buildServices(caps, hasMediaService, hasProfiles) {
    ...
    if (hasMediaService) {
        services.push("Media");
        if (hasProfiles)
            services.push("Media2");
    }
    if (Boolean(caps?.PTZ))
        services.push("PTZ");
    if (Boolean(caps?.Events))
        services.push("Events");
    if (Boolean(caps?.Imaging))
        services.push("Imaging");
    if (Boolean(caps?.Analytics))
        services.push("Analytics");
    if (Boolean(caps?.Recording))
        services.push("Recording");
    if (Boolean(caps?.DeviceIO))
        services.push("DeviceIO");
    if (Boolean(caps?.Replay))
