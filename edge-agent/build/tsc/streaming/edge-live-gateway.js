...
export class EdgeLiveGateway {
        ...
        this.server = createServer((request, response) => {
            void this.handle(request, response).catch((error) => {
                ...
                if (!response.headersSent)
                    sendJson(response, 502, { error: "media_gateway_failure" });
                else
    ...
    async listen(input) {
        ...
        if (!address || typeof address === "string")
            throw new Error("Edge live gateway did not bind a TCP address");
    ...
    async handle(request, response) {
        ...
        if (request.method === "POST" && url.pathname === "/v1/live/start") {
            ...
            if (!sourceUri)
                return sendJson(response, 503, { error: "stream_secret_unavailable" });
...
export function buildEdgeLiveGateway(options) {
...
export async function startEdgeMediaRuntime(input) {
    ...
    try {
        if (config.MEDIA_TUNNEL_MODE === "quick") {
            const started = await startQuickTunnel(config.CLOUDFLARED_PATH, `http://${config.EDGE_LIVE_GATEWAY_HOST}:${config.EDGE_LIVE_GATEWAY_PORT}`, runtimeDirectory);
        ...
        if (!resolvedPublicUrl)
            throw new Error("No public media gateway URL was established");
...
class EdgeAccessRegistry {
    ...
    authenticate(token, path, action) {
        return action === "read" && [...this.sessions.values()].some((session) => session.path === path && session.expiresAt > Date.now() && secureEqual(session.token, token));
    }
    async expire(id) {
...
async function waitForPublicGateway(url, timeoutMs) {
    ...
    while (Date.now() < deadline) {
        try {
            if ((await fetch(url, { signal: AbortSignal.timeout(2_000) })).ok)
                return;
        }
        catch { /* retry */ }
        await delay(500);
...
function safeIdentifier(value) { return value.toLowerCase().replace(/[^a-z0-9_-]/g, "-"); }
