...
async function main() {
    ...
    const gateway = new GatewayClient(config.CONTROL_PLANE_URL, config.DEV_USER_ID, config.EDGE_BRIDGE_SHARED_KEY, config.CONTROL_PLANE_TIMEOUT_MS);
    const agentId = config.EDGE_AGENT_ID ?? (await gateway.register(config.BRANCH_ID, config.EDGE_AGENT_NAME, config.EDGE_AGENT_VERSION)).id;
    ...
    const cameraHeartbeat = initializeCameraHeartbeat(config.CONTROL_PLANE_URL, config.BRANCH_ID, agentId, config.DEV_USER_ID, config.FFPROBE_PATH, config.FFMPEG_PATH, config.EDGE_BRIDGE_SHARED_KEY);
        ...
        await Promise.all([
            ...
            ...recorderReports.flatMap(({ recorder, probe }) => {
                ...
                if (probe.hddStatus.length)
                    submissions.push(gateway.submitRecorderHdd(agentId, {
                ...
                if (probe.archiveEvidence.length)
                    submissions.push(gateway.submitRecorderArchive(agentId, {
                        branchId: config.BRANCH_ID, recorderId: recorder.id, observedAt, source,
