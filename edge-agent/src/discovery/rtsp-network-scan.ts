  ...
  for (const [interfaceName, addrs] of Object.entries(ifaces)) {
    ...
    for (const addr of addrs) {
      ...
      if (Number.isInteger(detectedPrefix) && detectedPrefix >= 20 && detectedPrefix <= 30) {
        ...
        // A /8-/19 corporate network is too broad to probe blindly, but
        // returning no range at all makes local discovery silently do
        // nothing. Scan the agent's own /24; remote/VLAN ranges remain an
...
export async function discoverRtspDevices(
  ...
  // The control plane supplies all port policy with the agent package. Do
  // not silently probe unrelated ports on a branch network.
  const rtspStreamingPorts = uniqueValidPorts(options.ports);
  const httpFingerprintPorts = uniqueValidPorts(options.recorderHttpPorts);
...
function uniqueValidPorts(ports: readonly number[]) {
