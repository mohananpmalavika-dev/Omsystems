...
if (hasArgument(argv, "--version")) {
  process.stdout.write("Sentinel Grid Edge Agent 0.1.29\n");
  process.exit(0);
}
...
async function scanBranch(options: { persistStreamSecrets?: boolean; target?: DeviceScanTarget } = {}) {
  ...
  if (options.target) {
    ...
    try {
      endpoints = await discoverOnvifDevices(config.DISCOVERY_TIMEOUT_MS);
    } catch (error) {
      // Some endpoint-security policies block UDP multicast. Continue with
      // the independently configured TCP/RTSP discovery path rather than
      // reporting a successful scan with no probe having run.
      endpoints = [];
      logger.warn("ONVIF WS-Discovery is unavailable; continuing with RTSP/recorder discovery", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
...
function parseDiscoveryPorts(value: string, name: string) {
  const ports = [...new Set(value.split(",").map((item) => Number(item.trim())))]
    .filter((port) => Number.isInteger(port) && port >= 1 && port <= 65_535);
  if (ports.length === 0) throw new Error(`${name}_must_include_a_valid_tcp_port`);
  return ports;
}
