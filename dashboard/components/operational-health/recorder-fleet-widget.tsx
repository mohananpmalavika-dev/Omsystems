...
import { Activity, Camera, Clock, Fingerprint, RefreshCw, Server, ServerOff } from "lucide-react";
...
export function RecorderFleetWidget({ detailed = false, autoRefresh = true, refreshToken }: { detailed?: boolean; autoRefresh?: boolean; refreshToken?: number }) {
  ...
  const [inspectingRecorderId, setInspectingRecorderId] = useState<string | null>(null);
  ...
  const summary = fleet?.summary ?? { total: 0, online: 0, offline: 0, degraded: 0, unknown: 0, recording: 0, partial: 0, stopped: 0, unverified: 0, affectedBranches: 0 };
  ...
  return <section className="card" aria-labelledby="recorder-fleet-title">
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h2 id="recorder-fleet-title" className="flex items-center gap-2 text-lg font-semibold"><Server size={20}/> DVR/NVR real-time status</h2><p className="mt-1 text-sm text-gray-600">Vendor API and ONVIF reachability reported by branch edge agents.</p></div><div className="flex gap-2"><a href="/maintenance/compatibility" className="btn-secondary flex items-center gap-1.5 text-xs"><Fingerprint size={14}/> Compatibility profiles</a><button className="btn-secondary flex items-center gap-2" onClick={() => void load()}><RefreshCw size={15} className={loading ? "animate-spin" : ""}/> Refresh</button></div></div>
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">{visible.map((recorder) => <article key={recorder.id} className={`rounded-lg border-2 p-4 ${recorderTone(recorder.status)}`}>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-xs"><div><p className="text-gray-500">Device</p><strong>{recorder.vendor} {recorder.model}</strong></div><div><p className="text-gray-500">Address</p><strong>{recorder.ipAddress ?? "Hidden"}</strong></div><div className="flex items-center gap-2"><Clock size={13}/><span>{recorder.latencyMs === null ? "--" : `${recorder.latencyMs}ms`}</span></div><div className="flex items-center gap-2"><Camera size={13}/><span>{recorder.connectedCameras ?? "--"}/{recorder.totalCameras ?? "--"} channels</span></div><div className="col-span-2 flex items-center gap-2"><Activity size={13}/><span>Recording: <strong>{recorder.recordingStatus}</strong>{recorder.recordingChannels !== null && recorder.recordingChannels !== undefined ? ` (${recorder.recordingChannels}/${recorder.totalCameras ?? "--"})` : ""}</span></div><div className="col-span-2"><p className="text-gray-500">Newest recorder media</p><strong>{recorder.lastRecordedAt ? new Date(recorder.lastRecordedAt).toLocaleString() : "Not verified"}</strong></div></div>
      {detailed && recorder.channels?.length ? <div className="mt-3 rounded border bg-white/70 p-2"><p className="mb-2 text-xs font-semibold">Channel evidence</p><div className="grid gap-1 text-[11px]">{recorder.channels.map((channel) => <div key={channel.id} className="grid grid-cols-[auto_1fr_auto] items-center gap-2"><span>CH {channel.sourceChannel}</span><span className={channel.connected === false ? "text-red-700" : "text-gray-600"}>{channel.connected === null ? "connectivity unknown" : channel.connected ? "connected" : "offline"}</span><strong className={channel.status === "recording" ? "text-emerald-700" : channel.status === "stopped" ? "text-red-700" : "text-gray-600"}>{channel.status}</strong></div>)}</div></div> : null}
      <div className="mt-3 pt-2 border-t flex items-center justify-between">
        {recorder.reasonCodes.length ? <p className="text-[11px] text-gray-500 truncate max-w-[200px]">{recorder.reasonCodes.join(" · ")}</p> : <span />}
        <button
          onClick={() => setInspectingRecorderId(recorder.id)}
          className="text-xs font-semibold text-blue-700 hover:text-blue-900 flex items-center gap-1 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded transition"
        >
    ...
    {inspectingRecorderId && (
      <RecorderProfileInspector
        recorderId={inspectingRecorderId}
        isOpen={Boolean(inspectingRecorderId)}
        onClose={() => setInspectingRecorderId(null)}
        onRefingerprintRequested={load}
      />
    )}
  </section>;
}
