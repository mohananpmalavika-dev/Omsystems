...
export function HoldToTalkButton({ cameraId, disabled, unsupportedReason }: {
  ...
  const begin = async () => {
    ...
    try {
      ...
      // Heartbeat every 10s to keep lease renewed during continuous broadcast
      heartbeatTimer.current = setInterval(() => {
        if (sessionRef.current) {
          fetch(`/v1/cameras/${encodeURIComponent(cameraId)}/talk-sessions/${encodeURIComponent(session.sessionId)}/heartbeat`, {
            method: "POST",
      ...
      processor.onaudioprocess = (event) => {
        ...
        // Compute RMS level for VU visualizer
        let sum = 0;
        ...
        const pcm = downsamplePcm16(channelData, context.sampleRate, session.audio.sampleRate);
  ...
  const flush = async () => {
    ...
    try {
      while (!releaseRequested.current && queue.current.length) {
        ...
        if (chunk) await sendTalkAudio(session, chunk);
      }
    } catch (cause) {
      setError(talkMessage(cause));
      setState("error");
  ...
  return (
    <button
      ...
      style={{
        position: "relative",
        boxShadow: state === "talking" && audioLevel > 5 ? `0 0 ${Math.min(12, audioLevel / 5)}px rgba(239, 68, 68, 0.7)` : undefined,
      }}
    >
      {state === "connecting" ? (
        ...
        audioLevel > 15 ? <Volume2 size={15} className="text-red-500 animate-pulse" /> : <Mic size={15} className="text-red-500" />
      ) : (
        <Mic size={15} />
      )}
      <span className="talk-label">
        {state === "talking" ? (audioLevel > 10 ? `Live ${audioLevel}%` : "Talking") : "Hold"}
      </span>
    </button>
  );
}
