/**
 * Delete All Cameras and Branch Edges Script
 * 
 * This script removes all cameras and edge agents (branch edges) from the database.
 * It performs a cascading deletion of related data including:
 * - Cameras and their resource nodes
 * - Edge agents (branch edges)
 * - Camera discoveries
 * - Device identities
 * - Recording jobs
 * - Live sessions
...
interface DeletionStats {
  ...
  cameraNodes: number;
  edgeAgents: number;
  cameraDiscoveries: number;
  deviceIdentities: number;
  recordingJobs: number;
  liveSessions: number;
  analyticsAlerts: number;
  recordingSegments: number;
  incidentCameras: number;
  cameraAccessRequests: number;
  cameraSpecifications: number;
  edgeActivationTokens: number;
  edgeScanJobs: number;
  edgeCommands: number;
  edgeManagedTunnels: number;
}
 ...
 */
async function getCurrentCounts(): Promise<Partial<DeletionStats>> {
  const client = await pool.connect();
  try {
    const counts: Partial<DeletionStats> = {};
    ...
    for (const { key, query } of queries) {
      try {
        ...
        counts[key as keyof DeletionStats] = result.rows[0]?.count || 0;
      } catch (error) {
        console.warn(`⚠️  Could not get count for ${key}:`, error instanceof Error ? error.message : error);
        counts[key as keyof DeletionStats] = 0;
      }
    }
 ...
 */
async function deleteAllCamerasAndEdges(): Promise<DeletionStats> {
  const client = await pool.connect();
...
async function main() {
  ...
  try {
    ...
    console.error("\n❌ Error during deletion:", error instanceof Error ? error.message : error);
