...
const acceptedUntil="2026-12-31";
const acceptedPackages=new Set(["archiver","archiver-utils","brace-expansion","exceljs","glob","minimatch","next","postcss","readdir-glob","rimraf","sharp","uuid","zip-stream","@xmldom/xmldom","adm-zip","fast-uri","nanoid","nodemailer","onnxruntime-node","passport-saml","xml-crypto","xml-encryption"]);
...
const blocking=[];const accepted=[];for(const item of Object.values(report.vulnerabilities??{})){if(!["high","critical"].includes(item.severity))continue;if(!acceptedPackages.has(item.name))blocking.push(`${item.name}:${item.severity}`);else accepted.push(`${item.name}:${item.severity}`);}
